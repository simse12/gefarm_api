...
try {
    ...
    // 3. Gestione e decriptazione del Codice Fiscale
    $meter_data['codice_fiscale_owner'] = null; // ⭐️ CORRETTO: Inizializza a null
    $meter_data['is_meter_owner'] = $is_meter_owner; // Aggiungi lo status alla risposta

    // ⭐️ CORRETTO: Rimosso il controllo su $is_meter_owner
    if (!empty($meter_data['cf_owner_encrypted'])) {
        try {
            // Decripta il CF
