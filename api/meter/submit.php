...
try {
    ...
    // Verifica l'esistenza, l'autorizzazione E LO STATUS is_meter_owner
    $stmt = $db->prepare("SELECT d.id, ud.is_meter_owner FROM gefarm_devices d
    ...
    // Impedisci l'azione se l'utente non è l'intestatario
    if (!$device_association['is_meter_owner']) {
        Response::unauthorized('Accesso negato. Solo l\'intestatario della fornitura può inserire i dati del contatore.');
    }
    ...
    // Inserisci nuovi dati contatore (corretto per cf_owner_encrypted e inserimento di inserted_by_user_id)
    $stmt = $db->prepare("INSERT INTO gefarm_device_meter_data
                         (device_id, cf_owner_encrypted, nome, cognome, indirizzo, 
                          zip_code, citta, provincia, pod, email, 
                          telefono, is_active, valid_from, inserted_by_user_id)
                          VALUES
                         (:device_id, :cf_encrypted, :nome, :cognome, :indirizzo, 
                          :zip_code, :citta, :provincia, :pod, :email,
                          :telefono, 1, NOW(), :inserted_by_user_id)");
                          
    $stmt->bindValue(':device_id', $device_id, PDO::PARAM_INT);
    $stmt->bindValue(':cf_encrypted', $encrypted_cf, PDO::PARAM_STR);
    ...
    // ✅ AGGIUNTO: Tracciamento dell'utente che ha inserito i dati
    $stmt->bindValue(':inserted_by_user_id', $auth_data->user_id, PDO::PARAM_INT); 
    ...
    if ($stmt->execute()) {
        ...
        $response_data = [
            ...
            'cf_status' => 'Encrypted and stored'
        ];
        
        Response::success($response_data, "Dati contatore salvati con successo e Chain2 attivato", 201);
    } else {
